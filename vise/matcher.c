...
static bool create_searches(UStringSearch **searches, UChar *haystack, int32_t haystack_len, UChar *needle, int32_t needle_len) {
    int32_t i = 0, pos = 0;
    UErrorCode status = U_ZERO_ERROR;
...
static bool match(UChar **items, int32_t *item_lengths, uint32_t item_count, UChar *needle, Match *match_results, int32_t *final_positions, int32_t needle_char_len, UChar *level1, UChar *level2, UChar *level3) {
    ...
    for (i = 0; i < (int32_t)item_count; i++) {
        ...
        if (!create_searches(searches, matches[i].haystack, matches[i].haystack_len, needle, needle_len)) goto end;
        matches[i].memo = memo;
...
static PyTypeObject MatcherType = { // {{{
    ...
    "matcher_native.Matcher",            /*tp_name*/
...
static struct PyModuleDef moduledef = {
	...
	"matcher_native",     /* m_name */
	"Native code implementation for subsequence matching",  /* m_doc */
	-1,                  /* m_size */
	matcher_methods,    /* m_methods */
	NULL,                /* m_reload */
	NULL,                /* m_traverse */
	NULL,                /* m_clear */
	free_module,                /* m_free */
...
PyInit_matcher_native(void) {
    ...
    if (PyType_Ready(&MatcherType) < 0) return NULL;
